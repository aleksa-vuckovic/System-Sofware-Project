...
#include "../inc/SectionHeaderTable.h"
#include <map>
	...
	*/
	...
	for (int i = 0; i < cnt; i++) {
		...
		for (auto it = data->begin(); it != data->end(); it++) {
			...
			data = rel->apply(data, it->first, symTab, localSegmentLocs);
...
std::string Linker::merge(std::ifstream* inputs[], int cnt) {
	...
	std::unordered_map<std::string, int> baseLocs;
	for (auto it = finalSizes.begin(); it != finalSizes.end(); it++) baseLocs.insert({ it->first, 0 });
	calculateSegmentLocations(&localData, &baseLocs, &localSegmentLocations);

	SymbolTable* symTab = getMergedSymbolTable(inputs, cnt, &localSegmentLocations);
	for (auto it = finalSizes.begin(); it != finalSizes.end(); it++) symTab->addSymbol(0, 'S', it->first, it->first);
	...
	for (int i = 0; i < cnt; i++) {
		...
		std::unordered_map<std::string, int>* localSegmentLocs = localSegmentLocations.at(i);
		for (auto it = data->begin(); it != data->end(); it++) {
			std::string partiallyRelocatedData = rels->at(it->first)->applyPartially(it->second, it->first, symTab, localSegmentLocs);
			if (finalData.count(it->first) == 0) finalData.insert({ it->first, "" });
			finalData[it->first] += partiallyRelocatedData;
		}
	}

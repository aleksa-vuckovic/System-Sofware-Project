#include "../inc/InstructionTranslator.h"
...
int InstructionTranslator::getSize(Instruction* ins, int* litoffset, int* reloffset) {
	*litoffset = *reloffset = -1;
	std::string m = ins->getMnemonic();
	if (m == "iret") return 8;
	if (m == "jmp" || m == "beq" || m == "bgt" || m == "bne") {
		*reloffset = 2;
		return 4;
	}
	else if (m == "call") {
	...
	if (m == "ld") {
		...
		if (t == Operand::MEM_LIT || t == Operand::MEM_SYM || t == Operand::REG_LIT || t == Operand::REG_SYM) {
			*litoffset = 2;
			return 8;
		}
		else return 4;
		...
		if (t == Operand::REG_DIR || t == Operand::REG_IND) {
			return 4;
		}
		else {
			*litoffset = 6;
...
std::string InstructionTranslator::translate(Instruction* ins, int* litoffset, int* reloffset) {
	*litoffset = *reloffset = -1;
	...
	if (m == "st") {
		...
		if (t == Operand::MEM_LIT || t == Operand::MEM_SYM) {
			*litoffset = 2;
			return std::string("82F0") + regA + "000";
		}
